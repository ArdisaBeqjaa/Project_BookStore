...
public class Librarian extends BillNumber {
	...
	double totalPrice = 0;
	public void printbill(ArrayList<String> name,ArrayList<Integer> quantities) throws IOException {

		PrintWriter op = new PrintWriter("Bill.txt"+billNumber++);
		ArrayList<Book> stockbooks = Methods.readListBook();
		int total_sum=0;
		for(int i=0;i<name.size();i++){
			op.write("\nBook name is: "+name.get(i)+"\nCosts: "+stockbooks.get(i).getSellingPrice()+billNumber++);
			total_sum+=stockbooks.get(i).getSellingPrice();
		}
		BillNumber.updateBooks(stockbooks);
		moneyMade+=totalPrice;
		System.out.println("money madde"+moneyMade);
		numberOfBills+=1;
		datesSold.add(new Date());
		moneyMadeDates.add(moneyMade);

		op.write("\nTotal price: "+total_sum+" Date: "+(new Date()).toString());
		op.close();
		}
